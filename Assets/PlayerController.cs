...
public class PlayerController : MonoBehaviour
{
    ...
    private Transform eyepoleloc;
    [SerializeField] private GameObject BloaterCollider;
    Vector2 respawnPoint = new Vector2(-170, 5);
    ...
    [SerializeField] private float freezeHit;
    private bool _startFreezeTimer = false;
    [SerializeField] private float downAttackForce;
    private bool _canHeavyAttackTwo;
    private bool _canAttack=true;
    private bool _canHeavyAttackOne=true;
    private bool _canHeavyAttack=true;
    EnemyHitCollision enemyHitCollision;
    ...
    private SetRespawnPoint eyepole;
    private void Awake()//Awake loads faster than start
    {
        ...
        eyepole = FindObjectOfType<SetRespawnPoint>();
    ...
    IEnumerator Respawn(float waitTime)
    {
        ...
        if(eyepole.thisRespawn)
        {
            respawnPoint = eyepole.location;
        }
        yield return new WaitForSeconds(waitTime);
        OnRespawn.Instance.Respawn();
    ...
    private void FixedUpdate()
    {
        ...
        if (attack.AttackSucess)
        {
            ...
            StartCoroutine(AttackFreeze(freezeHit));
            if (IsFacingRight)
            {
                Debug.Log("HHHH");
        ...
        EnemyHitCollision enemyScript = FindObjectOfType<EnemyHitCollision>();
        if (touchingDirections.IsOnEnemy||(enemyScript.AttackSucess&& enemyScript.applyForce) ||touchingDirections.IsOnBoss || (forceTimer < 0.2f && forceTimer > 0))
        {
            
            if (enemyScript.applyForce)
            {
                Debug.Log("It did this");
                Debug.Log(enemyScript.enemyName);
                enemy= GameObject.Find(enemyScript.enemyName).transform;
            }
            else
            {
            ...
            if (touchingDirections.IsOnBoss)
            {
                StartCoroutine(TempIgnoreCollision()); // one second rn
            }
            touchingDirections.IsOnBoss = false;
            touchingDirections.IsOnEnemy = false;
            EnemyHitCollision.Instance.AttackSucess = false;
    ...
    private IEnumerator TempIgnoreCollision()
    {
    ...
    {
        if (!Damageable.Instance.IsAlive && IsRespawning || MenuOpen||animator.GetBool("Block"))
        {
            IsMoving = false;
    ...
    IEnumerator AttackFreeze(float waitTime)
    {
        Time.timeScale = 0;
    ...
    private IEnumerator CanHeavyAttack(float time)
    {
    ...
    public void OnAttack(InputAction.CallbackContext context)
    {
        ...
        if ((_canAttack&&context.started && !Input.GetKey(KeyCode.S) && !touchingDirections.IsGrounded && !Input.GetKey(KeyCode.W) ) || (_canAttack && !Input.GetKey(KeyCode.W) && context.started && touchingDirections.IsGrounded))
        {
    ...
    private IEnumerator SecondHeavyAttack(float time)
    {
        _canHeavyAttackTwo = true;
        yield return new WaitForSeconds(time);
        _canHeavyAttackTwo = false;
    ...
    public void OnHeavyAttack(InputAction.CallbackContext context)
    {
        if (!Damageable.Instance.IsAlive || MenuOpen)
        { return; }
        if(context.started&&!_canHeavyAttackTwo)
        {
            _canHeavyAttackOne = true;
        }
        if(context.started&&!_canHeavyAttackTwo&&!context.canceled&&_canHeavyAttackOne && _canHeavyAttack)
        {
           
            animator.SetTrigger("HeavyAttack");
            
            StartCoroutine(SecondHeavyAttack(1f));
            _canHeavyAttackOne = false;
        }
        else if (context.started && _canHeavyAttackTwo && !context.canceled&&_canHeavyAttack)
        {
            ...
            //_canHeavyAttackOne=true;
            _canHeavyAttackTwo = false;
            StartCoroutine(CanHeavyAttack(0.5f));
        }
        else
        {
