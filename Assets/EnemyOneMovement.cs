...
public class EnemyOneMovement : MonoBehaviour
{
    ...
    private float attackTimer=1.01f;
    ...
    private void FixedUpdate()
    {
        ...
        else if ((!attack.AttackSucess && timeCounter > 0.3) || (!attack.AttackSucess && timeCounter == 0) || ((attack.EnemyTag != "Bloater" && timeCounter > 0.3)))
        {
            ...
            else if(collisionDetector.CanSeePlayer&&distance<3&&attackTimer>1)
            {
                attackTimer = 0;
        ...
        else if(attack.EnemyTag == (gameObject.name) || (timeCounter > 0 && timeCounter < 0.1f))
        {
            Debug.Log("Fuck");
